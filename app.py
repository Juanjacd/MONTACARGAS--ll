...
# --- Descarga RAW controlada (timeouts cortos) ---
...
BASE_STYLE = f"""
<style>
:root{{--bg:#fff;--panel:#f8fafc;--ink:#0f172a;--muted:#64748b;--border:#e5e7eb;--accent:#0ea5e9;--header-h:64px}}
...
html, body, [data-testid="stAppViewContainer"], [data-testid="stSidebar"]{{background:var(--bg)!important;color:var(--ink)!important}}
section[data-testid="stSidebar"]{{background:var(--panel)!important;border-right:1px solid var(--border)}}
div.hero{{margin:0;border:1px solid var(--border);border-radius:14px;padding:14px 16px;background:var(--panel)}}
h1.hero-title{{margin:0;line-height:1.15;font-weight:800;color:var(--ink);font-size:clamp(20px,2.6vw+8px,34px)}}
...
div.section{{border:1px solid var(--border);border-radius:12px;padding:10px 12px;background:var(--panel);margin:14px 0 8px}}
.kpi-card{{border:1px solid var(--border);background:var(--panel);border-radius:14px;padding:12px 14px;color:var(--ink);max-width:420px}}
[data-testid="collapsedControl"]{{position:fixed;top:10px;left:10px;z-index:2000}}
</style>
<div class="hero"><h1 class="hero-title">{APP_TITLE}</h1><div class="hero-sub">{APP_TAGLINE}</div></div>
"""
st.markdown(BASE_STYLE, unsafe_allow_html=True)

with st.sidebar:
    st.markdown("---")
    dark = st.checkbox("🌙 Modo oscuro", value=True)
...
TURNOS = {
    "Turno A": {"start": dtime(5,0), "end": dtime(13,55),
                "lunch": (dtime(8,20), dtime(9,0)), "fuel": (dtime(9,0), dtime(9,18))},
    "Turno B": {"start": dtime(14,0), "end": dtime(22,55),
                "lunch": (dtime(17,25), dtime(18,0)), "fuel": (dtime(18,0), dtime(18,18))},
}
LATE_B_CUTOFF = dtime(3,0)

ITEM_WAZ="WA-ZONE"; ITEM_BODEGA="BODEGA-INT"
EXT_ITEMS=["CALIDAD-OK","TRANSFER","INSPECCIÓN","CARPA",ITEM_WAZ,ITEM_BODEGA,"UBIC.SOBRESTOCK","REACOM.SOBRESTOCK"]
PALETTES={"Vibrante Tropical":{"CALIDAD-OK":"#00B894","TRANSFER":"#0984E3","INSPECCIÓN":"#6C5CE7","CARPA":"#E17055",ITEM_WAZ:"#FDCB6E",ITEM_BODEGA:"#636E72","UBIC.SOBRESTOCK":"#00CEC9","REACOM.SOBRESTOCK":"#A29BFE"}}
EXT_COLOR_MAP = PALETTES["Vibrante Tropical"]

# ---------- Normalización ----------
def _norm_nfd_ascii(s:str)->str:
    if s is None: return ""
    return unicodedata.normalize("NFD", str(s)).encode("ascii","ignore").decode()
def _norm_compact(s:str)->str: return re.sub(r"[^a-z0-9]+","",_norm_nfd_ascii(s).lower())
def _norm_tokens(s:str)->List[str]: return [t for t in re.split(r"[^a-z0-9]+", _norm_nfd_ascii(s).lower()) if t]

def pick_col(cols_map:dict,*aliases)->Optional[str]:
    norm2orig={_norm_compact(k):v for k,v in cols_map.items()}
    for alias in aliases:
        key=_norm_compact(alias)
...
def to_time(x):
    ...
    try:
        if hasattr(x,"hour"): return dtime(int(x.hour), int(getattr(x,"minute",0)), int(getattr(x,"second",0)))
    except: pass
    if isinstance(x,(int,float)) and not pd.isna(x):
        try:
            dtv=pd.to_datetime(x, unit="d", origin="1899-12-30"); return dtime(int(dtv.hour), int(dtv.minute), int(getattr(dtv,"second",0)))
        except: pass
    s=str(x).strip().replace("\u00A0"," ").replace("\u202F"," ")
    s=re.sub(r"\s+"," ",s)
    s=re.sub(r"(?i)\ba\.?\s*m\.?\b","AM",s); s=re.sub(r"(?i)\bp\.?\s*m\.?\b","PM",s)
    s=s.replace("a.m.","AM").replace("p.m.","PM").replace("a.m","AM").replace("p.m","PM")
    for fmt in ("%I:%M:%S %p","%I:%M %p","%H:%M:%S","%H:%M"):
        try: dtv=pd.to_datetime(s, format=fmt); return dtime(int(dtv.hour), int(dtv.minute), int(getattr(dtv,"second",0)))
        except: pass
    dtv=pd.to_datetime(s, errors="coerce")
    if pd.notnull(dtv): return dtime(int(dtv.hour), int(dtv.minute), int(getattr(dtv,"second",0)))
    return None
...
def has_003(s): s=str(s or "").upper().strip(); return s.startswith("003") or bool(re.search(r'(^|[^0-9])003([^0-9]|$)', s))
def looks_like_slot_code(s):
    s=str(s or "").upper().strip()
    if not s or s.startswith("003"): return False
    return bool(re.fullmatch(r'[0-9A-Z][0-9A-Z/-]*', s))
def _contains_any(txt,patterns): return any(p in txt for p in patterns)
def _item_base(ubic_proced, ubic_dest):
    up,ud=str(ubic_proced or ""), str(ubic_dest or "")
    both=f"{up}|{ud}"; compact=_norm_compact(both); toks=set(_norm_tokens(both))
...
CANON_MAP={"calidad ok":"CALIDAD-OK","calidad-ok":"CALIDAD-OK","transfer":"TRANSFER","inspeccion":"INSPECCIÓN","inspección":"INSPECCIÓN","inspection":"INSPECCIÓN","inspec":"INSPECCIÓN","insp":"INSPECCIÓN","insp.":"INSPECCIÓN","carpa":"CARPA","carpas":"CARPA","wa zone":"WA-ZONE","wa-zone":"WA-ZONE","wazone":"WA-ZONE","bodega int":"BODEGA-INT","bodega-int":"BODEGA-INT","movimiento en bodega":"BODEGA-INT","ubic.sobrestock":"UBIC.SOBRESTOCK","ubicacion sobrestock":"UBIC.SOBRESTOCK","reacom.sobrestock":"REACOM.SOBRESTOCK","reacomodacion sobrestock":"REACOM.SOBRESTOCK"}
def _norm_label(s): s=_norm_nfd_ascii(str(s or "")).lower(); s=re.sub(r"[^a-z0-9\s\-_/\.]+"," ",s).strip(); s=re.sub(r"\s+"," ",s); return s
def canon_item_from_text(s):
    t=_norm_label(s); 
    ...
    return None
def item_ext(ubic_proced, ubic_dest):
    up=str(ubic_proced or "").strip(); ud=str(ubic_dest or "").strip()
    up003,ud003=has_003(up),has_003(ud)
...
def make_uid(row):
    """
    Genera un ID estable por fila con:
    Usuario | Fecha | Datetime (redondeado al minuto) | Orden
    """
    dtv = pd.to_datetime(row.get("Datetime"), errors="coerce")
    dtv_floor = (None if pd.isna(dtv) else dtv.floor("min"))
    base = f"{row.get('Usuario','')}|{row.get('Fecha','')}|{str(dtv_floor)}|{str(row.get('Orden') or '')}"
...
def read_all(path)->pd.DataFrame:
    ...
    try: df=pd.read_sql_query(f"SELECT * FROM {TABLE}", con)
    except Exception: df=pd.DataFrame(columns=["id","usuario","fecha","time","turno","datetime","orden","ubic_proced","ubic_destino","itemraw"])
    con.close()
    if df.empty: return df
    df.rename(columns={"usuario":"Usuario","fecha":"Fecha","time":"TimeStr","turno":"Turno","datetime":"Datetime","orden":"Orden","ubic_proced":"Ubic.proced","ubic_destino":"Ubicación de destino","itemraw":"ItemRaw"}, inplace=True)
...
def load_excel(file, sheet_name="Hoja1")->pd.DataFrame:
    ...
    col_item   =pick_col(cols,"item","ítem","movimiento","tipo movimiento","actividad","proceso","tarea","categoria","clase","operacion","detalle movimiento","detalle del movimiento","mov","nombre movimiento")
    ...
    out["Datetime"]=out.apply(lambda r: pd.Timestamp.combine(r["Fecha"], r["Time"]) if (pd.notnull(r["Fecha"]) and r["Time"] is not None) else pd.NaT, axis=1)
...
def load_excel_from_github_raw(raw_url:str)->pd.DataFrame:
    ...
    r.raise_for_status()
    return load_excel(io.BytesIO(r.content), sheet_name="Hoja1")
    ...
    with st.expander("📥 Carga de datos", expanded=True):
        ...
        if not auto_local:
            ...
            if up is not None:
                ...
                except: hoja=st.text_input("Hoja", value="Hoja1")
                finally:
        ...
        # Botón manual SIEMPRE visible (máxima prioridad)
...
EXT_COLOR_MAP = PALETTES[st.session_state.get("pal_name","Vibrante Tropical")]

# ---------- Carga con anti-bloqueo ----------
df_new = pd.DataFrame()

# 0) Manual fijo
if up_fixed is not None:
    try:
        with st.spinner("Leyendo Excel subido..."):
...
# 1) RAW (solo si no hay manual)
if df_new.empty:
    try:
        with st.spinner("Descargando desde GitHub RAW..."):
            df_new = load_excel_from_github_raw(GITHUB_RAW_URL)
        st.success("Datos cargados automáticamente desde GitHub RAW ✅")
    except Exception as e:
        st.info(f"No se pudo descargar RAW ({e}). Probando ruta local...")

# 2) Local (si existe)
if df_new.empty and auto_local and os.path.exists(local_path):
    try:
...
# 3) Uploader condicional (si desmarcaste auto_local)
if df_new.empty and (up is not None):
    try:
        with st.spinner("Leyendo archivo del uploader..."):
            df_new = load_excel(up, hoja)
        st.success("Datos cargados del uploader (manual) ✅")
    except Exception as e:
        st.error(f"❌ No pude leer el Excel subido: {e}")
        st.stop()
...
# ---------- Fecha operativa & Turno defensivo ----------
def apply_oper_day(dfin:pd.DataFrame)->pd.DataFrame:
    d=dfin.copy()
    ...
    d["DatetimeOper"]=d.apply(lambda r: pd.Timestamp.combine(pd.Timestamp(r["FechaOper"]).date(), r["Time"]) if (pd.notnull(r["FechaOper"]) and r["Time"] is not None) else pd.NaT, axis=1)
    return d

df = apply_oper_day(df)
mask_ab = df["Turno"].isin(["Turno A","Turno B"])
if mask_ab.any():
    df = df[mask_ab].copy()
else:
    st.warning("No pude clasificar turnos (¿Hora vacía/formato distinto?). Muestro datos sin filtrar por turno.")
    df["Turno"]=df["Turno"].fillna("—")

# ---------------- Filtros (acepta 1 día o rango) ----------------
with st.sidebar:
...
def _short_label(user:str, turnoAB:str)->str: return f"{str(user).strip()}-{turnoAB}"
def _responsive_bar_style(fig, n:int):
    if n<=6: bargap,bgrp=0.12,0.05
    elif n<=12: bargap,bgrp=0.10,0.04
    elif n<=24: bargap,bgrp=0.08,0.03
    else: bargap,bgrp=0.06,0.02
    fig.update_layout(bargap=bargap, bargroupgap=bgrp)

def render_kpis(df_filtered:pd.DataFrame):
    total=len(df_filtered); uniq=df_filtered["Usuario"].nunique()
    tA=int((df_filtered["Turno"]=="Turno A").sum()); tB=int((df_filtered["Turno"]=="Turno B").sum())
    st.markdown(f"""<div class="kpi-card">
    <div class="kpi-title">📊 Indicadores</div>
    <div>Órdenes: <b>{total:,}</b></div>
    <div>Usuarios únicos: <b>{uniq}</b></div>
    <div>Turno A: <b>{tA:,}</b> — Turno B: <b>{tB:,}</b></div>
    </div>""", unsafe_allow_html=True)

def view_tm_por_usuario_turno():
    render_section_title("Tiempo Muerto — dos barras por Usuario (Turno A/B), apilado por ítem")
    dtmp=df_f.copy()
    ...
    EXC=[TURNOS["Turno A"]["lunch"],TURNOS["Turno A"]["fuel"],TURNOS["Turno B"]["lunch"],TURNOS["Turno B"]["fuel"]]
    rows=[]
    for _,r in df_g.iterrows():
        start=r["prev_dt"]; end=r["DatetimeOper"]; date=r["FechaOper"]
        gap=(end-start).total_seconds()/60.0
        if gap<=0: continue
        def sub_win(s,e,ws,we):
            if we<=s or ws>=e: return [(s,e)]
            out=[]; 
            if s<ws: out.append((s,max(s,ws)))
            if e>we: out.append((min(e,we),e))
            return [(a,b) for (a,b) in out if b>a]
        segs=[(start,end)]
        for st_t,en_t in EXC:
            st_w=pd.Timestamp.combine(pd.Timestamp(date), st_t)
            en_w=pd.Timestamp.combine(pd.Timestamp(date), en_t)
            tmp=[]; 
            for s,e in segs: tmp+=sub_win(s,e,st_w,en_w)
            segs=tmp
            if not segs: break
    ...
    if sel_items: tm_ut=tm_ut[tm_ut["ItemExt"].isin(sel_items)]
    g=tm_ut.copy(); g["TurnoAB"]=g["Turno"].str.replace("Turno ","", regex=False)
    ...
    order_axis=[]; present=set(g["UsuarioTurnoShort"])
    for u in order_users:
        for ab in ["A","B"]:
            k=_short_label(u,ab)
            if k in present: order_axis.append(k)
    ...
    if chart_is_h:
        ...
        fig=px.bar(g, x="Min", y="UsuarioTurnoShort", color="ItemExt", orientation="h",
                   barmode="stack", category_orders={"UsuarioTurnoShort":order_axis}, color_discrete_map=EXT_COLOR_MAP,
                   custom_data=["ItemExt","Min"], height=height)
        fig.update_traces(hovertemplate="Ítem: %{customdata[0]}<br>Min: %{customdata[1]:.0f}<extra></extra>", marker_line_width=0, opacity=0.95, cliponaxis=False)
        ...
        xmax=max(1,float(totals.max())); fig.update_xaxes(range=[0,xmax*1.06])
        _responsive_bar_style(fig, len(order_axis)); fig.update_layout(margin=dict(t=10,b=10,l=10,r=110), legend_title_text="Ítem")
    else:
        height=max(420, 24*len(order_axis)+60)
        fig=px.bar(g, x="UsuarioTurnoShort", y="Min", color="ItemExt", barmode="stack",
                   category_orders={"UsuarioTurnoShort":order_axis}, color_discrete_map=EXT_COLOR_MAP,
                   custom_data=["ItemExt","Min"], height=height)
        fig.update_traces(hovertemplate="Ítem: %{customdata[0]}<br>Min: %{customdata[1]:.0f}<extra></extra>", marker_line_width=0, opacity=0.95, cliponaxis=False)
        fig.update_xaxes(categoryorder="array", categoryarray=order_axis, tickangle=(-65 if len(order_axis)>8 else -30))
        totals=(g.groupby("UsuarioTurnoShort")["Min"].sum().reindex(order_axis))
        ymax=float(totals.max())*1.18; fig.update_yaxes(range=[0,ymax])
        fig.add_trace(go.Bar(x=totals.index.tolist(), y=totals.values, marker_color='rgba(0,0,0,0)', showlegend=False, hoverinfo="skip",
                             text=[f"{v:.0f}" for v in totals.values], textposition="outside", textfont=dict(size=10, color=ANN_COL), cliponaxis=False))
        _responsive_bar_style(fig, len(order_axis)); fig.update_layout(margin=dict(t=10, b=10, l=10, r=10), legend_title_text="Ítem")
...
def view_ordenes_ot():
    ...
    if cnt.empty: st.info("No hay órdenes en el filtro actual."); return
    ...
    order_axis=[]; present=set(cnt["UsuarioTurnoShort"])
    for u in order_users:
        for ab in ["A","B"]:
            k=_short_label(u,ab)
            if k in present: order_axis.append(k)
    n=len(order_axis); tick_angle=-65 if n>8 else -30
    if n>14:
        height=max(440, 22*n+120)
        fig=px.bar(cnt, y="UsuarioTurnoShort", x="CNT", color="ItemExt", barmode="stack",
                   category_orders={"UsuarioTurnoShort":order_axis}, color_discrete_map=EXT_COLOR_MAP,
                   custom_data=["ItemExt","CNT","UsuarioTurnoShort"], height=height, orientation="h")
        fig.update_traces(hovertemplate="Ítem: %{customdata[0]}<br>Órdenes: %{customdata[1]:.0f}<br>%{customdata[2]}<extra></extra>", marker_line_width=0, opacity=0.95, cliponaxis=False)
        ...
        fig.add_trace(go.Scatter(x=totals.values, y=totals.index.tolist(), mode="text",
                                 ...
                                 showlegend=False, hoverinfo="skip"))
        xmax=max(1,float(totals.max())); fig.update_xaxes(range=[0,xmax*1.08])
        _responsive_bar_style(fig, n); fig.update_layout(margin=dict(t=40,b=10,l=10,r=110), legend_title_text="Ítem")
    else:
        height=max(520, 26*n+100)
        fig=px.bar(cnt, x="UsuarioTurnoShort", y="CNT", color="ItemExt", barmode="stack",
                   category_orders={"UsuarioTurnoShort":order_axis}, color_discrete_map=EXT_COLOR_MAP,
                   custom_data=["ItemExt","CNT","UsuarioTurnoShort"], height=height)
        fig.update_traces(hovertemplate="Ítem: %{customdata[0]}<br>Órdenes: %{customdata[1]:.0f}<br>%{customdata[2]}<extra></extra>", marker_line_width=0, opacity=0.95, cliponaxis=False)
        fig.update_xaxes(categoryorder="array", categoryarray=order_axis, tickangle=tick_angle)
        totals=(cnt.groupby("UsuarioTurnoShort")["CNT"].sum().reindex(order_axis))
        y_max=float(totals.max())*1.18; fig.update_yaxes(range=[0,y_max], automargin=True)
        _responsive_bar_style(fig, n); fig.update_layout(margin=dict(t=50,b=10,l=10,r=100), legend_title_text="Ítem")
    apply_plot_theme(fig); c1,c2=st.columns([3,1]); 
...
# =========================================================
def minutes_of_day(ts:pd.Timestamp)->float: t=ts.time(); return t.hour*60+t.minute+t.second/60.0
def fmt_hhmm(m): m=int(round(m)); return f"{m//60:02d}:{m%60:02d}"
def minutes_for_plot(ts,turno): 
    m=minutes_of_day(ts)
    if turno=="Turno B" and ts.time()<LATE_B_CUTOFF: m+=24*60
    return m
...
def view_inicio_fin_turno():
    ...
    d=df_pre.copy()
    if "ItemExt_any" not in d.columns: d["ItemExt_any"]=d.apply(classify_any_row, axis=1)
    recs=[]
    for (usr, fecha_op, turno), g in d.sort_values("DatetimeOper").groupby(["Usuario","FechaOper","Turno"]):
        if g.empty: continue
        g=g.copy(); g["t_vis"]=g["DatetimeOper"].apply(lambda ts: minutes_for_plot(ts, turno))
        r_ini=g.loc[g["t_vis"].idxmin()]; t_ini=float(r_ini["t_vis"]); it_ini=r_ini["ItemExt_any"]
        lunch_start,_=TURNOS.get(turno,TURNOS["Turno A"])["lunch"]; lunch_m=lunch_start.hour*60+lunch_start.minute
        g_pre=g[g["t_vis"]<lunch_m]
        if not g_pre.empty:
            r_al=g_pre.loc[g_pre["t_vis"].idxmax()]; t_alim=float(r_al["t_vis"]); it_alim=r_al["ItemExt_any"]
        else: t_alim=None; it_alim=None
        r_cie=g.loc[g["t_vis"].idxmax()]; t_cie=float(r_cie["t_vis"]); it_cie=r_cie["ItemExt_any"]
        had_extra=bool(g["IsExtra"].any())
        recs.append({"Usuario":usr,"Turno":turno,"FechaOper":fecha_op,"t_ini":t_ini,"t_alim":t_alim,"t_cie":t_cie,"it_ini":it_ini,"it_alim":(it_alim if t_alim is not None else "—"),"it_cie":it_cie,"extra":had_extra})
    if not recs: st.info("No se pudieron calcular hitos con el filtro actual."); return
    dd=pd.DataFrame(recs); one_day=(dd["FechaOper"].nunique()==1)
    agg_rows=[]
    for (usr, turno), g in dd.groupby(["Usuario","Turno"]):
        if one_day:
            r=g.iloc[-1]; n=1; t_ini=r["t_ini"]; t_alim=r["t_alim"]; t_cie=r["t_cie"]; it_ini=r["it_ini"]; it_alim=r["it_alim"]; it_cie=r["it_cie"]; modo="Día único"; extra_days=int(bool(r["extra"])); extra_info=("Sí" if r["extra"] else "No")
        else:
            n=g["FechaOper"].nunique(); t_ini=g["t_ini"].mean(skipna=True); t_alim=g["t_alim"].mean(skipna=True); t_cie=g["t_cie"].mean(skipna=True)
            it_ini=most_common(g["it_ini"].tolist()); it_alim=most_common(g["it_alim"].tolist()); it_cie=most_common(g["it_cie"].tolist())
            modo=f"Promedio de {n} días"; extra_days=int(g["extra"].sum()); extra_info=f"{extra_days} de {n} días con extra"
        agg_rows.append({"Usuario":usr,"Turno":turno,"n_dias":n,"modo":modo,"t_ini":t_ini,"t_alim":t_alim,"t_cie":t_cie,"it_ini":it_ini,"it_alim":it_alim,"it_cie":it_cie,"extra_days":extra_days,"extra_info":extra_info})
    ...
    for _,r in agg.iterrows():
        t_ini=r["t_ini"]; t_alim=r["t_alim"] if pd.notna(r["t_alim"]) else None; base_for_end=t_alim if t_alim is not None else t_ini
        seg_ini=max(0.0,float(t_ini)); seg_alim=max(0.0,float(t_alim-t_ini)) if t_alim is not None else 0.0; seg_cie=max(0.0,float(r["t_cie"]-base_for_end))
        rows += [
            {"UsuarioTurnoShort":r["UsuarioTurnoShort"],"Hito":"Inicio","Seg":seg_ini,"Hora":fmt_hhmm(t_ini),"Info":r["modo"],"Item":r["it_ini"],"Extra":r["extra_info"]},
            {"UsuarioTurnoShort":r["UsuarioTurnoShort"],"Hito":"Antes de alimentación","Seg":seg_alim,"Hora":fmt_hhmm(t_alim) if t_alim is not None else "—","Info":r["modo"],"Item":(r["it_alim"] if t_alim is not None else "—"),"Extra":r["extra_info"]},
            {"UsuarioTurnoShort":r["UsuarioTurnoShort"],"Hito":"Antes de cierre","Seg":seg_cie,"Hora":fmt_hhmm(r["t_cie"]),"Info":r["modo"],"Item":r["it_cie"],"Extra":r["extra_info"]},
        ]
    m=pd.DataFrame(rows); top=m.groupby("UsuarioTurnoShort")["Seg"].sum().reindex(order_axis)
    has_extra=(agg["extra_days"]>0).any(); base_max=27*60 if has_extra else 24*60
    y_max=max(base_max, int(top.max()//60 + 2)*60); ticks=list(range(0,y_max+1,60)); ticktext=[fmt_hhmm(t) for t in ticks]
    ...
    if many:
        ...
        if len(order_axis)<=12:
            fig.add_trace(go.Scatter(x=top.index.tolist(), y=top.values, mode="text",
                                     text=[fmt_hhmm(v) for v in top.values], textposition="top center",
                                     textfont=dict(size=11, color=ANN_COL), showlegend=False, hoverinfo="skip"))
